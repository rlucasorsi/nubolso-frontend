...
function getDeadlineText(goal: Goal) {
  ...
  if (percent >= 90) {
    return {
      icon: PartyPopper,
      text: diffDays > 0 ? `Quase lá! Conclusão em ${diffDays} dias` : 'Meta atingida!',
      highlight: true,
    };
  }

  if (diffDays < 0) {
    return { icon: Clock, text: 'Prazo vencido', highlight: false };
  }
  ...
  if (diffDays <= 365) {
    ...
    return {
      icon: Calendar,
      text: `Previsão: ${months[deadline.getMonth()]} ${deadline.getFullYear()}`,
      highlight: false,
    };
  }
  ...
  return {
    icon: Timer,
    text: `Meta de longo prazo (${deadline.getFullYear()})`,
    highlight: false,
  };
}
...
export function GoalCard({ goal, onClick, onAddFunds }: GoalCardProps) {
  ...
  const percent = Math.round((goal.savedAmount / goal.targetAmount) * 100);
  const deadlineInfo = getDeadlineText(goal);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-muted-foreground line-clamp-1">
              {goal.description || 'Meta Financeira'}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-end">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Acumulado</span>
              <span className="text-lg font-bold text-foreground">{formatCurrencyFull(goal.savedAmount)}</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Objetivo</span>
